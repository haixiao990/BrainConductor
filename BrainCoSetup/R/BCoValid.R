             ...
             filters=NULL, silent=FALSE, ..., fix=FALSE)
{
    ...
    oldPkgs <- old.packages(lib.loc, repos=BCoInstallRepos(),
    ...
    if (fix) {
        ...
        BCoInstall(pkgs, lib.loc=lib.loc, ...)
...
##        print(x$oldPkgs)
##        cat("\nupdate with BCoInstall()\n\n")
##    }

##    if (NROW(x$tooNewPkgs)) {
##        cat("* Packages too new for BrainConductor version ",
##            .sQuote(as.character(BCoInstall())), "\n\n", sep="")
##        print(x$tooNewPkgs)
##        pkgs <- paste(.dQuote(rownames(x$tooNewPkgs)), collapse=", ")
##        msg <- .msg(ifelse(NROW(x$tooNewPkgs) == 1L, "BCoInstall(%s)",
##                           "BCoInstall(c(%s))"), pkgs)
##        cat("\ndowngrade with ", msg, "\n\n", sep="")
##    }
##}
