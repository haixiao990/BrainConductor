...
analysis, and display of neuroimaging data, such as SPM\cite{penny2011statistical}, FSL\cite{jenkinson2012fsl}, FreeSurfer\cite{fischl2012freesurfer} and AFNI\cite{cox1996afni}. NITRC also is
...
\caption{Drawbacks of NITRC framework, description of the drawbacks and proposed solution
...
of this file format that could be encountered by analysts. First, neuroimaging data files often comprise a large amount of
...
space. Human Connectome Project (HCP) had a similar solution by defining the CIFTI
...
phenotype information is stored separately in a different file, and many statistical
...
\begin{center}
	...
	Includes functions for creating 3D and 4D images using WebGL, RGL, and JavaScript Commands \\\hline
	...
	Functions for the input/output and visualization of data in the ANALYZE, NIfTI or AFNI formats. \\\hline
	...
	Enables analysis and visualisation of 3D biological image data, especially traced neurons \\\hline
	nat.templatebrains & NeuroAnatomy Toolbox Extension for Handling Template Brains &
	Extends package 'nat'  by providing objects and functions for handling template brains. \\
	\hline
    \end{tabular}
    \caption{fMRI Packages Incorporated into the BrainConductor Project {\color{red}Why is the font different...}}
    \label{tab:neuro}
\end{center}
\end{footnotesize}
\end{table}
...
\begin{center}
    ...
    \hline
	Package Name & Full Name & Description \\ \hline
	bigdata & Big Data Analytics & Provides a LASSO regression using stability selection.
	for large-scale data analysis \\\hline
	fastclime & A Solver for Parameterized Linear Programming to Precision Matrix Estimation &
	An efficient method of recovering precision matrices by applying the parametric simplex method is provided in this package.  \\\hline
	flare & Family of Lasso Regression & 
	Provides the implementation of a family of Lasso variants including Dantzig Selector, LAD Lasso, SQRT Lasso, Lq Lasso for estimating high dimensional sparse linear model. \\\hline
	huge & High-Dimensional Undirected Graph Estimation & 
	Provides a general framework for high-dimensional undirected graph estimation.\\\hline
	picasso &  Pathwise Calibrated Sparse Shooting Algorithm & Implements the pathwise calibrated sparse shooting algorithm regularized sparse linear regression, sparse logistic regression, and sparse undirected graphical model estimation. \\\hline
	smart & Sparse Multivariate Analysis via Rank Transformation & Provides a general framework for analyzing (including estimation, feature selection and prediction) and visualize big data. \\\hline
    \end{tabular}
...
The base installation of BrainConductor also comes with standardized imaging resources
used by the neuroscience community. These include datasets such
as the Montreal Neurological
Institute (MNI) brain atlases, the Automated Anatomical Labeling (AAL) parcellation,
tissue prior,
...
dedicated effort
to ensure streamlined compatibility of these imaging resources with our NIdata format.
...
%The definition of functional connectomics\cite{sporns2011human,
%biswal2010toward, kelly2012characterizing} relied on the statistical
%association of different voxels or region-of-interests (ROIs). In statistics,
%it is an issue of the computation and inference of large-scale graphic model.
%The problem of widely-used Pearson correlation matrix... We use ... Our results
%showed ...    
...
%Maintainers are responsible for testing new or updated packages and published
%these packages on the server and website. A check of completed documents is
%also needed. The work may be trivial but is necessary. \\
...
%If we found it is necessary, we can define a new S4 class as a uniform data
%format in BrainConductor. The standards of the development of R packages for
%BrainConductor can take CRAN as a reference. I don't think that designing new
%rules is a good idea before we have a good enough reputation , because
%developers may just ignore these rules and submit their packages to CRAN
%instead of BrainConductor. 

%A \textit{.hdr} file consists of 348 bytes data and usually 4 bytes nulls,
%including some basic information, such as the number of voxels at 3 dimensions,
%the data type (unsigned character, integer, float, and double), and the number
%of time points for time serials data like fMRI. Each \textit{.img} file saved
%an image at one time point. A DICOM folder is the output data format directly
%from the previous machines and software. A Nifti (\textit{.nii}) file, which is
%often called 4dnifti, is a sequential junction of these files. Although these
%data formats are stored as binary data with uncertain data type and are
%difficult to be read by R language directly, we should utilize these data
%formats as a standard data structure as they are standards in this field. The
%good news is that there has already been some packages providing functions to
